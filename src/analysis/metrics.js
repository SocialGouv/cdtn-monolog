...
const clean = (visits) => {
  ...
  const cleanSeriesOfVisits = seriesOfVisits.map((x) =>
    x.transformSeries({
...
const analyse = (dataset, reportId) => {
  ...
  const visitorSelectedRelated = selectRelatedStats.map(
    (x) => x["visitorSelectedRelated"]
  );
  const SelectRelatedCount = selectRelatedStats.map(
    (x) => x["selectRelatedCount"]
  );
  const visitorWasRedirected = selectRelatedStats.map(
    (x) => x["visitorWasRedirected"]
  );
